...
# have max length `TERMUX__INTERNAL_NAME_MAX_LEN`.
#
...
# `TERMUX__INTERNAL_NAME_MAX_LEN` is not checked by this regex and
...
# `TERMUX__APPS_APP_IDENTIFIER_MAX_LEN` is not checked by this regex
...
# as its limited by `TERMUX__APPS_DIR_MAX_LEN` and `TERMUX__ROOTFS_DIR_MAX_LEN`.
#
...
__termux_build_props__validate_variables() {
    ...
    if [ "${#TERMUX__INTERNAL_NAME}" -gt ${TERMUX__INTERNAL_NAME_MAX_LEN:?} ]; then
        echo "The TERMUX__INTERNAL_NAME '$TERMUX__INTERNAL_NAME' with length ${#TERMUX__INTERNAL_NAME} is invalid." 1>&2
        echo "The TERMUX__INTERNAL_NAME must have max length \`<= TERMUX__INTERNAL_NAME_MAX_LEN ($TERMUX__INTERNAL_NAME_MAX_LEN)\`." 1>&2
        return 1
    fi
...
variable name '$variable_name' is invalid." 1>&2
    ...
    if [[ "$__TERMUX_BUILD_PROPS__VALIDATE_PATHS_MAX_LEN" == "true" ]] && \
            [ "${#TERMUX_APP__DATA_DIR}" -ge ${TERMUX_APP__DATA_DIR_MAX_LEN:?} ]; then
        echo "The TERMUX_APP__DATA_DIR '$TERMUX_APP__DATA_DIR' with length ${#TERMUX_APP__DATA_DIR} is invalid." 1>&2
        echo "The TERMUX_APP__DATA_DIR must have max length \`<= TERMUX_APP__DATA_DIR_MAX_LEN ($TERMUX_APP__DATA_DIR_MAX_LEN)\` \
including the null \`\0\` terminator." 1>&2
        return 1
    fi

    if [[ "$__TERMUX_BUILD_PROPS__VALIDATE_PATHS_MAX_LEN" == "true" ]] && \
            [ "${#TERMUX__APPS_DIR}" -ge ${TERMUX__APPS_DIR_MAX_LEN:?} ]; then
        echo "The TERMUX__APPS_DIR '$TERMUX__APPS_DIR' with length ${#TERMUX__APPS_DIR} is invalid." 1>&2
        echo "The TERMUX__APPS_DIR must have max length \`<= TERMUX__APPS_DIR_MAX_LEN ($TERMUX__APPS_DIR_MAX_LEN)\` \
including the null \`\0\` terminator." 1>&2
        return 1
    fi
    ...
    if [[ "$__TERMUX_BUILD_PROPS__VALIDATE_PATHS_MAX_LEN" == "true" ]] && \
            [ "${#TERMUX_APP__IDENTIFIER}" -gt ${TERMUX__APPS_APP_IDENTIFIER_MAX_LEN:?} ]; then
        echo "The TERMUX_APP__IDENTIFIER '$TERMUX_APP__IDENTIFIER' with length ${#TERMUX_APP__IDENTIFIER} is invalid." 1>&2
        echo "The TERMUX_APP__IDENTIFIER must have max length \
\`<= TERMUX__APPS_APP_IDENTIFIER_MAX_LEN ($TERMUX__APPS_APP_IDENTIFIER_MAX_LEN)\`." 1>&2
        return 1
    fi


    if [[ "$__TERMUX_BUILD_PROPS__VALIDATE_PATHS_MAX_LEN" == "true" ]] && \
            [ "${#TERMUX__ROOTFS}" -ge ${TERMUX__ROOTFS_DIR_MAX_LEN:?} ]; then
        echo "The TERMUX__ROOTFS '$TERMUX__ROOTFS' with length ${#TERMUX__ROOTFS} is invalid." 1>&2
        echo "The TERMUX__ROOTFS must have max length \`<= TERMUX__ROOTFS_DIR_MAX_LEN ($TERMUX__ROOTFS_DIR_MAX_LEN)\` \
including the null \`\0\` terminator." 1>&2
        return 1
    fi
